...
function makeMeasurement(float $temp = 20.0, float $humidity = 50.0, float $pressure = 1013.0): Measurement
{
    return Measurement::create(
        ...
        reportedAt:          new \DateTimeImmutable('2026-04-01T12:00:00Z'),
    );
}
...
test('creates measurement with correct data', function () {
    ...
    expect($measurement->temperature()->value())->toBe(20.0)
        ...
        ->and($measurement->stationName())->toBe('Estación Central')
...
test('station name is immutable after update', function () {
    $measurement = makeMeasurement();

    $measurement->update(
        new Temperature(30.0),
        new Humidity(60.0),
        new AtmosphericPressure(1010.0),
        new \DateTimeImmutable('2026-04-02T12:00:00Z'),
    );

    expect($measurement->stationName())->toBe('Estación Central');
});
